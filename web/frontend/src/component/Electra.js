...
class Electra extends Component {
    ...
    handleChange = e => {
        ...
        switch (question) {
            case 'question':
                errors.question = value.length === 0 ? 'Lütfen Sorunuzu Giriniz!' : '';
                break;
            case 'context':
    ...
    handleSubmit = async (e) => {
        ...
        if (document.mainForm.question.value === "") {
            ...
            const electra_endpoint = "qa_electra"
            const response = await qa_api.post(`${electra_endpoint}`,body)
            if(response.data != ''){
    ...
    render() {
        ...
        return (
            <div>
                <div>
                    <p className="title">ELECTRA Modelimiz</p>
                </div>
